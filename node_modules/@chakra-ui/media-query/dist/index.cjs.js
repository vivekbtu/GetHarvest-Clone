...
function useMediaQuery(query, options = {}) {
  ...
  (0, import_react2.useEffect)(() => {
    ...
    mql.forEach((mql2) => {
      if ((0, import_utils.isFunction)(mql2.addListener))
    ...
    return () => {
      mql.forEach((mql2) => {
        if ((0, import_utils.isFunction)(mql2.removeListener))
...
var getBreakpoint = (theme, value) => (0, import_utils2.memoizedGet)(theme, `breakpoints.${value}`, value);
...
function useBreakpoint(arg) {
  var _a;
  const opts = (0, import_utils3.isObject)(arg) ? arg : { fallback: arg ?? "base" };
...
function getClosestValue(values, breakpoint, breakpoints = import_utils4.breakpoints) {
...
function useBreakpointValue(values, arg) {
  var _a;
  const opts = (0, import_utils5.isObject)(arg) ? arg : { fallback: arg ?? "base" };
  const breakpoint = useBreakpoint(opts);
  const theme = (0, import_system3.useTheme)();
  if (!breakpoint)
    return;
  const breakpoints = Array.from(((_a = theme.__breakpoints) == null ? void 0 : _a.keys) || []);
  const obj = (0, import_utils5.isArray)(values) ? (0, import_utils5.fromEntries)(Object.entries((0, import_utils5.arrayToObjectNotation)(values, breakpoints)).map(([key, value]) => [key, value])) : values;
