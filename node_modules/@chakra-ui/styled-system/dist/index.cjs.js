...
var withoutImportant = (value) => (0, import_utils.isString)(value) ? value.replace(/!(important)?$/, "").trim() : value;
var tokenToCSSVar = (scale, value) => (theme) => {
  const valueStr = String(value);
  const important = isImportant(valueStr);
  const valueWithoutImportant = withoutImportant(valueStr);
  const key = scale ? `${scale}.${valueWithoutImportant}` : valueWithoutImportant;
  let transformed = (0, import_utils.isObject)(theme.__cssMap) && key in theme.__cssMap ? theme.__cssMap[key].varRef : value;
...
var isCSSFunction = (value) => {
  return (0, import_utils2.isString)(value) && value.includes("(") && value.includes(")");
};
...
var transformFunctions = {
  ...
  px(value) {
    ...
    return unitless || (0, import_utils3.isNumber)(value) ? `${value}px` : value;
  },
  fraction(value) {
    return !(0, import_utils3.isNumber)(value) || value > 1 ? value : `${value * 100}%`;
  },
  float(value, theme) {
  ...
  degree(value) {
    if ((0, import_utils3.isCssVar)(value) || value == null)
      return value;
    const unitless = (0, import_utils3.isString)(value) && !value.endsWith("deg");
    return (0, import_utils3.isNumber)(value) || unitless ? `${value}deg` : value;
  },
...
var getWithPriority = (theme, key, styles) => {
  const result = {};
  const obj = (0, import_utils14.memoizedGet)(theme, key, {});
...
function resolveReference(operand) {
  if ((0, import_utils23.isObject)(operand) && operand.reference) {
...
var pseudoPropNames = (0, import_utils24.objectKeys)(pseudoSelectors);
...
function createThemeVars(flatTokens, options) {
  ...
  for (const [token, tokenValue] of Object.entries(flatTokens)) {
    ...
    const normalizedValue = (0, import_utils25.isObject)(value) ? value : { default: value };
    cssVars = (0, import_utils25.mergeWith)(cssVars, Object.entries(normalizedValue).reduce((acc, [conditionAlias, conditionValue]) => {
      var _a;
...
function extractTokens(theme) {
  ...
  return (0, import_utils26.pick)(theme, _tokens);
}
...
function flattenTokens({
  ...
  const tokenEntries = Object.entries((0, import_utils27.flatten)(tokens2) ?? {}).map(([token, value]) => {
    const enhancedToken = { isSemantic: false, value };
    return [token, enhancedToken];
  });
  const semanticTokenEntries = Object.entries((0, import_utils27.flatten)(semanticTokens, 1) ?? {}).map(([token, value]) => {
    const enhancedToken = { isSemantic: true, value };
    return [token, enhancedToken];
  });
  return (0, import_utils27.fromEntries)([...tokenEntries, ...semanticTokenEntries]);
}
...
function toCSSVar(rawTheme) {
  ...
  Object.assign(theme, {
    ...
    __cssMap: cssMap,
    __breakpoints: (0, import_utils28.analyzeBreakpoints)(theme.breakpoints)
  });
  return theme;
}
...
var systemProps = (0, import_utils29.mergeWith)({}, background, border, color, flexbox, layout, filter, ring, interactivity, grid, others, position, effect, space, scroll, typography, textDecoration, transform, list, transition);
var layoutSystem = Object.assign({}, space, layout, flexbox, grid, position);
var layoutPropNames = (0, import_utils29.objectKeys)(layoutSystem);
var propNames = [...(0, import_utils29.objectKeys)(systemProps), ...pseudoPropNames];
var styleProps = { ...systemProps, ...pseudoSelectors };
var isStyleProp = (prop) => prop in styleProps;

// src/css.ts
var isCSSVariableTokenValue = (key, value) => key.startsWith("--") && (0, import_utils30.isString)(value) && !(0, import_utils30.isCssVar)(value);
...
function getCss(options) {
  ...
  const css2 = (stylesOrFn, nested = false) => {
    var _a;
    const styles = (0, import_utils30.runIfFn)(stylesOrFn, theme);
    let computedStyles = {};
    for (let key in styles) {
      let value = (0, import_utils30.runIfFn)(styles[key], theme);
      if (value == null)
        continue;
      if (Array.isArray(value) || (0, import_utils30.isObject)(value) && isResponsive(value)) {
      ...
      if ((0, import_utils30.isObject)(value)) {
      ...
      if (!nested && (config == null ? void 0 : config.static)) {
        const staticStyles = (0, import_utils30.runIfFn)(config.static, theme);
      ...
      if ((0, import_utils30.isObject)(rawValue)) {
        computedStyles = Object.assign({}, computedStyles, rawValue);
        continue;
      }
      const configProperty = (0, import_utils30.runIfFn)(config == null ? void 0 : config.property, theme);
      if (configProperty) {
        ...
        if (configProperty === "&" && (0, import_utils30.isObject)(rawValue)) {
          computedStyles = Object.assign({}, computedStyles, rawValue);
        } else {
...
function normalize(value, toArray) {
  if ((0, import_utils31.isArray)(value))
    return value;
  if ((0, import_utils31.isObject)(value))
...
function createResolver(theme) {
  ...
  return function resolver(config, prop, value, props) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      const query = (0, import_utils31.toMediaQueryString)(key.minW, nextKey == null ? void 0 : nextKey._minW);
      const styles = (0, import_utils31.runIfFn)((_a = config[prop]) == null ? void 0 : _a[normalized[i]], props);
      if (!styles)
        continue;
      if (isMultipart) {
        (_b = config.parts) == null ? void 0 : _b.forEach((part) => {
          (0, import_utils31.mergeWith)(result, {
      ...
      if (!isMultipart) {
        if (isSingle)
          (0, import_utils31.mergeWith)(result, styles);
        else
          result[query] = styles;
        continue;
      }
...
function resolveStyleConfig(config) {
  return (props) => {
    ...
    return (0, import_utils31.mergeWith)({}, (0, import_utils31.runIfFn)(config.baseStyle ?? {}, props), recipe(config, "sizes", size, props), recipe(config, "variants", variant, props));
  };
}
...
function omitThemingProps(props) {
  return (0, import_utils32.omit)(props, ["styleConfig", "size", "variant", "colorScheme"]);
}
